...
typedef enum {
	...
	divide, mod, power, not, eos, operand //eos 앞에 power와 not 추가
} precedence;

int icp[] = { 20, 19, 12, 12, 13, 13, 13, 17, 18, 0 }; //스택 밖 연산자 우선 순위
int isp[] = { 0, 19, 12, 12, 13, 13, 13, 17, 18, 0 }; //스택 안 연산자 우선 순위
...
precedence get_token(char* symbol, int* n) //expr함수에서도 작동해야함!!
